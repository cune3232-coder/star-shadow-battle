...
export const Card: React.FC<CardProps> = ({ card, onClick, isSmall = false, isFaceDown = false, isSelected = false, className = '' }) => {
    ...
    return (
        <div
            ...
            className={`
                relative ${isSmall ? 'w-10 h-14 text-[8px]' : 'w-24 h-36 text-xs'}
                rounded-lg border-2 shadow-xl backdrop-blur-sm
                flex flex-col p-2 select-none transition-all duration-200
                ${baseStyle}
                ${isSelected ? '-translate-y-6 scale-110 ring-4 ring-yellow-400 shadow-[0_0_20px_rgba(250,204,21,0.6)] z-30' : 'hover:-translate-y-2 hover:brightness-110'}
        ...
        >
            ...
            {/* Body */}
            <div className="flex-1 flex items-center justify-center text-center leading-snug opacity-90 text-[10px] overflow-hidden">
